...
class Dataset():
    def __init__(self, num_pts=1024):
        self.num_pts = num_pts

        self.cam_cx = 322.52500
        self.cam_cy = 244.11084
        self.cam_fx = 591.01250
    ...
    def projection(self, img_dir, current_r, current_t,bbox, index):
        ...
        for tg in bbox:
            ...
            for xxx in range(x - 2, x + 3):
                for yyy in range(y - 2, y + 3):
                    img[xxx][yyy] = self.color[9]

        # imageio.imsave('results/{0}.png'.format(self.index), img)
        im = Image.fromarray(img)

        im.save('results/{0}.png'.format(index))
