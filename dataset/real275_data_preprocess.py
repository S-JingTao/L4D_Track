...
from utils.nocs_real_data import load_depth


def create_img_list(data_dir):
    """ Create train/val/test data list for Real275. """
    # Real dataset
...
def create_img_list_class(data_dir):
    """ Create train/val/test data list for each category in Real275. """
    # Real dataset
    cat_names = ['bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']
    obj_name_list = {}
    class_num_list = [1, 2, 3, 4, 5, 6]

    for i in range(len(class_num_list)):
        for subset in ['train', 'val']:
            results = {'bottle': [], 'bowl': [], 'camera': [], 'can': [], 'laptop': [], 'mug': []}
            obj_name_list[class_num_list[i]] = os.listdir(
                '{0}/data_list_real/real_{1}/{2}/'.format(data_dir, subset, class_num_list[i]))
            for item in obj_name_list[class_num_list[i]]:
                input_file = open(
                    '{0}/data_list_real/real_{1}/{2}/{3}/list.txt'.format(data_dir, subset, class_num_list[i], item),
                    'r')
                while 1:
                    ...
                    if subset == "val":
                        results[cat_names[i]].append(
                            '{0}/{1}/{2}'.format("test", input_line.split('/')[1], input_line.split('/')[-1]))
                    else:
                        results[cat_names[i]].append(
                            '{0}/{1}/{2}'.format(subset, input_line.split('/')[1], input_line.split('/')[-1]))
            with open(os.path.join(data_dir, 'Real', '{0}_{1}_list.txt'.format(subset, cat_names[i])), 'w') as f:
                for img_path in results[cat_names[i]]:
                    f.write("%s\n" % img_path)
    print('Write all category data paths to file done!')
...
def annotate_test_data(data_dir):
    ...
    camera_val = open(os.path.join(data_dir, 'CAMERA', 'val_list.txt')).read().splitlines()
    ...
    for source, img_list, intrinsics, subset in subset_meta:
        ...
        if source == "CAMERA":
            continue
        print("cur processing: {}".format(source))
        for img_path in tqdm(img_list):
            ...
            nocs_dir = os.path.join(os.path.dirname(data_dir), 'gts/6D-ViT_results')
            if source == 'CAMERA':
